...
class PID:
    def __init__(self, set_point, pid_min, pid_max, Kp, Ki, Kd, start_from_min=False):
        ...
        self.sum_e = 0
        self.value = None
        self.output_pwm = (self.max + self.min) / 2
        if start_from_min:
            self.output_pwm = self.min
        self.start_output_pwm = self.output_pwm
    ...
    def reset(self):
        ...
        self.output_pwm = self.start_output_pwm
        self.last_e = None
        self.sum_e = 0
        self.time = time.time()
    ...
    def calculate(self):
        ...
        if self.value is not None:
            ...
            output_ppm = self.start_output_pwm + PID_sum
